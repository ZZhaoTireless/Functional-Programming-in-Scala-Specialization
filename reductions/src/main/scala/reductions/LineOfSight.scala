...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    input.zipWithIndex.tail.map(i => i._1 / i._2).scan(input.head)((x, y) => max(x, y)).copyToArray(output)
  }
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    input.slice(from, until).zip(Stream from from).map(i => i._1 / i._2).scan(0f)((x, y) => max(x, y)).last
  }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float], startingAngle: Float, from: Int, until: Int): Unit = {
    val heading = max(input(from) / from, startingAngle)
    input.slice(from + 1, until).zip(Stream from from + 1).map(i => i._1 / i._2).scan(heading)((x, y) => max(x, y)).copyToArray(output, from, until - from)
  }
