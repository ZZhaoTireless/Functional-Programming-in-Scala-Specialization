...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    ...
    else means.map(p => p -> GenSeq[Point]()).toMap
  }
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    oldMeans.zip(newMeans).aggregate(true)((previousCheckResult, pointTuple) =>
      if (pointTuple._1.squareDistance(pointTuple._2) <= eta) previousCheckResult else false,
      _ && _
    )
  }
